...
import { initialProfile, rollbackToVersion } from "../src/core/profileEngine";
...
describe("correction engine", () => {
  ...
  it("approves durable guidance and preserves stable proposal identity after factual edits", () => {
    ...
    expect(result.profile.currentVersion).toBe(2);
    expect(result.profile.versions[0].rules).toEqual([]);
    const rolledBack = rollbackToVersion(result.profile, 1);
    expect(rolledBack.rules).toEqual([]);
  });
